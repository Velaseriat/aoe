...
public sealed class AlphaTrayContext : ApplicationContext
{
    ...
    public AlphaTrayContext()
    {
        ...
        menu.Items.Add($"AOE Alpha - dictation 0x{_config.PushToTalkVk:X2} / assistant 0x{_config.AssistantPushToTalkVk:X2}").Enabled = false;
        ...
        _tray.BalloonTipText = $"Running in the system tray. Dictation: 0x{_config.PushToTalkVk:X2}, Assistant: 0x{_config.AssistantPushToTalkVk:X2}; connecting to Beta...";
    ...
    private static string Describe(AlphaStatus status, string? detail) => status switch
    {
        ...
        AlphaStatus.Recording => detail is null ? "recording" : $"recording ({detail})",
        AlphaStatus.Thinking => "thinking (assistant)",
