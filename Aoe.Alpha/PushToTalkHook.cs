...
/// <summary>
/// Global low-level keyboard hook (WH_KEYBOARD_LL) that reports clean key-down / key-up for one or
/// more virtual-keys. Must be installed on a thread that pumps Windows messages (the UI thread).
/// </summary>
public sealed class PushToTalkHook : IDisposable
{
    ...
    private readonly HashSet<int> _down = new(); // keys currently held (suppresses auto-repeat)
    private readonly LowLevelKeyboardProc _proc; // kept alive to prevent GC of the callback
    private IntPtr _hook = IntPtr.Zero;

    /// <summary>Raised once when a target key transitions up-&gt;down; carries the virtual-key code.</summary>
    public event Action<int>? Pressed;

    /// <summary>Raised when a target key is released; carries the virtual-key code.</summary>
